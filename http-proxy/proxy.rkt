#lang racket
  ...
  (require file/gunzip)
(require openssl)
[require [file "C:/Users/user/Dropbox/actinic/src/actinic/actinic.ss"]]
[define [debug text]
  ;[displayln text]
  #t]
[define possible-title "No title detected"]

[define banlist [list "biz" "addthis" "imageshack.us" "amazon"  "paypal" "doubleclick" "AdSense" "api.stats" "google_analytics"]]
;[define banlist [list ]]
...
[define start-relay-server-to-client [lambda [filename server-readp client-writep rangeStart]
(let-values [[(in out) (make-pipe)]]
   ...
   [no-thread [thunk [with-handlers ([[thunk* #f] [lambda [err] [displayln [format "Thread closed: ~a" err] ]#f]]) 
                       ...
                       [let [[fileport [if [and range filename] [open-output-file  filename #:mode 'binary #:exists 'can-update] #f]]]
                           ...
                           [if [equal? "304"  [first headers]]
                               ...
                               [begin
                                 ...
                                 [flush-output client-writep]
                                   ...
                                   [when [and [assoc  "Content-Encoding" [fourth headers]] [equal? "gzip" [cdr [assoc  "Content-Encoding" [fourth headers]]]]]    
                                   [letrec [[contentport [open-output-bytes]]]
                         ...
                         [begin 
                           [my-copy-port-length [make-buffer] server-readp client-writep  fileport #f [string->number [cdr[assoc  "Content-Length" [fourth headers]]]]]
                         [and [assoc "Connection" [fourth headers]] [equal? "Keep-Alive" [cdr [assoc "Connection" [fourth headers]]]]]]]]
                         [begin
...
[define [build-request data]
  ...
  [let [[request [format "~a ~a ~a~a~a~a" [third data] [jrl-path [first data]] [second data] crlf
                     [string-join
          ...
          [begin
            [if [equal? [third request] "CONNECT"]
                [begin
                  ...
                  [display [build-response `[  "404" "Fuck off"  "HTTP/1.1" [,[cons "Proxy-agent" "HURFDURF"  ],[cons "Content-Type" "application/tunnel"],[cons "Connection" "close"]]]] client-writep]
                ...
                [begin
                  [letrec [[url [first request]]
                    ...
                    [let-values [[[server-readp server-writep] (tcp-connect  [jrl-server [first request]] [jrl-port [first request]] )]]
                      ...
                      [if [start-relay-server-to-client 
                           [if capture_traffic 
                               [format "~a\\~a" 
                                       log_dir 
