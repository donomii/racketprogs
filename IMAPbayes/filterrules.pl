...
my @filters = (
[ "INBOX/aaaa", sub { local $_ = shift; if ($_->to) {if ($_->sender) {my @to = map {$_->{mailbox}."@".$_->{host}} @{$_->sender}; my $to = join(",", @to); return $to =~ /aaaa/; }}} ],
[ "INBOX/bbbb", sub { local $_ = shift; if ($_->to) {if ($_->sender) {my @to = map {$_->{mailbox}."@".$_->{host}} @{$_->sender}; my $to = join(",", @to); return $to =~ /bbbb/i; }}} ],
[ "INBOX/ccc", sub { local $_ = shift; if ($_->to) {if ($_->sender) {my @to = map {$_->{mailbox}."@".$_->{host}} @{$_->sender}; my $to = join(",", @to); return $to =~ /cccc/; }}} ],
);
