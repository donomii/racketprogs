...
/* This module was written to supply the functionality of a text editor, including multiple edit buffers.
It can be used to editor moderate sized files, or as a simple one-line edit box.

There are two main functions: dispatch, which accepts commands like "END-OF-LINE" and "PAGE-DOWN" and executes them, and ActiveBufferInsert, which accepts a string and inserts it into the active buffer.

A new editor can be created with NewEditor.
*/
...
func SaveFile(gc *GlobalConfig, fname string, txt string) {
	...
	err := ioutil.WriteFile(fname, []byte(txt), 0o644)
...
func ActiveBufferText(gc *GlobalConfig) string {
	return gc.ActiveBuffer.Data.Text
}
