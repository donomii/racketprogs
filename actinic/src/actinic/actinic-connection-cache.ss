    ...
    (class* object% (cache<%>)
      ...
      (define trim-cache
        (lambda ()
          ...
          (if (> (car (cache-fill)) (cadr (cache-fill)))
              [begin
                [displayln "Cache too large, trimming"](hash-table-map cache (lambda (a-key a-val) (if (< 0 (random 4)) (hash-table-remove! cache a-key))))])
          ;(semaphore-post sem)
          ))
      ...
      (define/public fetch
        (lambda (a-key a-thunk)
          ...
          (let ([res (hash-table-get cache a-key #f)])
            [if res
                res
                [let [[res [a-thunk]]]
                  (add a-key res)
